...
\section{V2 Follow-Up: Maintenance Scheduling Empirical Comparison (E-L3A + E-L3B)}
\label{sec:v2_scheduling_followup}

Subsequent work re-formulated the Layer~3 maintenance scheduling task as a full QUBO over $x_{m,e,s} \in \{0,1\}$ decision variables (machine $m$ assigned to employee $e$ at time slot $s$) under industrial-realistic constraints (working hours 09:00--18:00 Mon--Fri in 60-minute slots, employee groups of 1--20 staff, sequential multi-task, failure-type-specific maintenance durations from CMMS literature: Mobley 2002; Gulati 2013; ISO 17359:2018). Six solvers were benchmarked across 315 controlled instances and 135 dynamic re-optimisation events. Implementation: \texttt{experiments/E-L3A\_static\_scheduling/} and \texttt{experiments/E-L3B\_dynamic\_scheduling/}. Pre-registration: \texttt{phase\_docs/PHASE1\_PROTOCOL\_V2\_ALIGNED\_3LAYER.md}.

\subsection{E-L3A Static Scheduling Comparison}

Six solvers compared across three problem sizes (Small 5 machines $\times$ 1 day; Medium 20 machines $\times$ 3 days; Large 50 machines $\times$ 5 days) $\times$ three employee-group sizes $\times$ five random seeds = 45 controlled instances:

\begin{table}[htbp]
\centering
\caption{E-L3A solver comparison: mean makespan and feasibility rate across 5 seeds. $\dagger$ = infeasible.}
\label{tab:el3a_comparison}
\begin{tabular}{lrrrrrrr}
\hline
\textbf{Size} & \textbf{N} & \textbf{E} & \textbf{Greedy / GA / SA / TS} & \textbf{Feas (cls)} & \textbf{QA / SBM} & \textbf{Feas (qtm)} \\
\hline
Small  & 5  & 5  & 2.6 (all converge) & 1.000 & 10.4$\dagger$ / 10.4$\dagger$ & 0.0 / 0.0 \\
Medium & 20 & 5  & 8.2 (all converge) & 1.000 & 28.4$\dagger$ / 28.6$\dagger$ & 0.0 / 0.0 \\
Large  & 50 & 20 & 5.2 (all converge) & 1.000 & n/a (skip) & --- \\
\hline
\end{tabular}
\end{table}

\textbf{Honest empirical finding:} all four classical heuristics (Greedy, Genetic Algorithm, Simulated Annealing, Tabu Search) converge to identical optimal makespan with 100\% feasibility on all tested cells; sub-second wall-clock at the largest tested scale (50 $\times$ 5 $\times$ 9 = 22500--45000 binary variables). Two evaluated quantum-class methods --- simulated Quantum Annealing via dwave-neal Ising mode (\textbf{distinct from classical SA}) and quantum-inspired Simulated Bifurcation per Goto et al.\ 2019 (\emph{Sci.\ Adv.}) --- consistently fail to produce feasible solutions on the dense scheduling QUBO encoding (paired Wilcoxon signed-rank $p < 10^{-5}$ vs classical-best on 25 paired observations). Gate-model QAOA on Qiskit Aer statevector simulator is \textbf{not applicable} at meaningful problem sizes due to the qubit budget ($\sim$18 qubits practical, versus $\geq$45 variables required at the smallest meaningful instance).

This V2 result \emph{complements} the chapter's earlier classical-heuristic disclosure: the earlier sections honestly disclosed that the existing pipeline components labelled ``quantum'' were classical heuristics; the V2 evaluation tested whether \emph{actual} simulator-based quantum-class methods (dwave-neal QA, SBM) and gate-model QAOA could produce competitive scheduling solutions on a properly formulated QUBO. The answer at simulator-only evaluation: \textbf{they cannot}, on this dense encoding at QASAMAP scale. Real D-Wave Advantage hardware was sought but the Leap free tier was not available at runtime; real-NISQ-hardware replication is reserved as future work.

\subsection{E-L3B Dynamic Real-Time Feasibility}

Given the E-L3A finding, E-L3B narrowed scope to validate whether classical solvers can sustain real-time feasibility under event-driven re-optimisation. Simulation: 8-hour working day (09:00--17:00) at Medium scale (20 initial machines, 5 employees, 3-day horizon), Poisson arrivals at $\lambda = 2$/hour, plus stochastic employee finish-early / delay events; 3 seeds $\times$ $\sim$15 events = 45 re-optimisation events.

\begin{table}[htbp]
\centering
\caption{E-L3B real-time feasibility: per-solver across 45 dynamic re-optimisation events.}
\label{tab:el3b_realtime}
\begin{tabular}{lrrrrr}
\hline
\textbf{Solver} & \textbf{Re-opts} & \textbf{Feas rate} & \textbf{Wall median (s)} & \textbf{Wall p95 (s)} & \textbf{Exceed 60s} \\
\hline
Greedy & 45 & 1.000 & $<$0.001 & $<$0.001 & 0 \\
SA     & 45 & 1.000 & 0.09 & 0.12 & 0 \\
GA     & 45 & 1.000 & 0.69 & 0.92 & 0 \\
TS     & 45 & 1.000 & 1.07 & 1.36 & 0 \\
\hline
\end{tabular}
\end{table}

All 4 classical solvers PASS real-time feasibility with $>$40$\times$ margin to the 60-s budget. Feasibility rate 1.000 across 135 re-optimisation events. Combined with the V2 V3 ensemble Layer~2 latency (e2e p95 = 32.7~s, see Chapter~5 §\ref{sec:el2_v3_followup}), the integrated Layer~2 + Layer~3 pipeline operates within the 60-s real-time Kafka producer cycle by $\geq 26$~s margin for any classical Layer~3 solver choice.

\subsection{Reconciliation with Cerezo et al.\ 2025 Quantum-ML Skepticism}

The 2025 wave of quantum-machine-learning skepticism, anchored by Cerezo et al.\ 2025 (\emph{Nature Communications}), demonstrates that variational quantum circuits without barren plateaus may be classically simulable. The V2 empirical findings in this thesis are consistent with that skepticism on this specific problem class:

\begin{itemize}
    \item Gate-model QAOA on dense scheduling QUBO requires more qubits than practical statevector simulators support.
    \item Simulated Quantum Annealing and quantum-inspired Simulated Bifurcation consistently fail to produce feasible solutions on this problem encoding, even after raising the constraint penalty $\lambda$ from 100 to 5000 and num\_reads to 1000.
    \item Industrial precedents (Ford Otosan + D-Wave 6$\times$ scheduling speedup; BASF + D-Wave 7200$\times$ liquid-filling) demonstrate practical advantage on \emph{specific} scheduling problem classes that differ structurally from QASAMAP's industrial maintenance scheduling.
\end{itemize}
...
This chapter presented the quantum-inspired classical heuristic layer and its mathematical formulations. The Pauli-Z multi-sensor encoding produces a composite risk indicator $\langle Z \rangle_c$ that detects sub-threshold risk combinations missed by classical single-sensor thresholds. The QUBO-formulated local search identifies Machine~32 as the highest-priority machine (energy $-0.5144$), and the QAOA-formulated graph scheduling partitions the 7-machine pool into urgent and planned maintenance slots. The heuristic override mechanism triggers for 5 out of 7 machines, expanding active decision coverage from 14.3\% to 85.7\%. \textbf{Critical disclosure:} The components disclosed in §\ref{sec:v2_scheduling_followup} (\texttt{energy\_distance\_anomaly\_score}, \texttt{random\_constrained\_search}, \texttt{graph\_message\_passing\_score}) are classical heuristics that preserve quantum mathematical structures for future hardware migration; none are executed on quantum processors.

The V2 follow-up (§\ref{sec:v2_scheduling_followup}) re-formulated the Layer~3 scheduling task as a full $x_{m,e,s}$ QUBO under industrial-realistic constraints and benchmarked six solvers (4 classical + 2 quantum-class) across 315 controlled instances and 135 dynamic re-optimisation events. The honest empirical finding at simulator-only evaluation: classical heuristics dominate quantum-class methods on this dense scheduling QUBO, while all four classical solvers sustain real-time feasibility within the 60-s Kafka producer cycle. Combined with the Layer~2 V3 ensemble e2e p95 latency of 32.7~s, the integrated Layer~2 + Layer~3 pipeline operates within real-time budget by $\geq 26$~s margin. Real D-Wave Advantage hardware testing is reserved as future work pending Leap programme access.
